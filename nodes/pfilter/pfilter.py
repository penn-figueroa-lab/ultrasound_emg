...
def t_noise_only(x, sigmas, df=1.0):
...
def half_half_sample(fn_list):
    ...
    def sample_fn(n):
        result = np.empty((n // 2 * 2, 0)).T
        for i in range(1, len(fn_list)):
            if i % 2 == 1:
                result = np.vstack(
                    (result, np.hstack((fn_list[i - 1](n // 2), fn_list[i](n // 2))))
                )
        return result.T
...
class ParticleFilter(object):
    ...
    def __init__(
        ...
        self.plot_sample = 5
        self.particle_alive_time = np.zeros(self.n_particles, dtype=np.int32)

        self.n_eff = 0.5 # just to have something to start with
        # self.ellipsoid_matrix = ellipsoid_matrix or identity

    def weighted_kde(self):
        data = self.original_particles[:, 0:3]
        grid_points = (10,20,20)
        grid, points = FFTKDE(bw=0.1/2, kernel='gaussian').fit(data, weights=self.weights_vis).evaluate(grid_points)
        # find the max point grid 
        max_idx = np.argmax(points, axis=None)
        # print("max idx is", max_idx, "max point is: ", grid[max_idx,0], grid[max_idx,1], grid[max_idx,2])
        
        # This maybe not a great idea, A estimation is questionable 
        # This uses A_est from particles, not the A_est from the attractor
        data_A = self.original_particles[:, [3, 7, 11]]
        grid_points_A = (10,10,10)
        grid_A, points_A = FFTKDE(bw=0.2/2, kernel='gaussian').fit(data_A, weights=self.weights_vis).evaluate(grid_points_A)
        # find the max point grid
        max_idx_A = np.argmax(points_A, axis=None)
        A_max = np.diag(grid_A[max_idx_A,:])
        # print("max A is", A_max)
        return A_max, grid[max_idx,:].reshape(-1, 1)

    def aggregate_fun(self):
        # attractor = self.aggregate_attractor()
        # A, attractor = self.weighted_kde()
        A = self.mean_state[3:12].reshape(3,3)
        attractor = self.mean_state[0:3].reshape(3,1)
        return A, attractor

    def plot_DS(self, ax):
        # if A is None: # means we are estimating A, plot the A in the particle using a aggregate function
        A, attractor = self.aggregate_fun()
        A = A[0:2, 0:2]
        attractor = attractor[0:2]
        attractor = attractor.reshape(-1)
        ds_x, ds_y = np.meshgrid(
            np.linspace(0.0, 5.0, self.plot_sample),
            np.linspace(0.0, 5.0, self.plot_sample),
        )
        ds_X = np.vstack([ds_x.ravel(), ds_y.ravel()])
        dx = np.zeros((2, self.plot_sample**2))
        for i in range(self.plot_sample**2):
            # f_g = np.zeros((2, self.n_particles))
            # for k in range(self.n_particles):
            #     f_g[:,k] = self.weights_vis[k] * (A @ (ds_X[:,i] - self.original_particles[k,0:2]))
            # f_g = np.sum(f_g, axis=1)
            dx[:, i] = A @ (ds_X[:, i] - attractor)

        uu = dx[0, :].reshape((self.plot_sample, self.plot_sample))
        vv = dx[1, :].reshape((self.plot_sample, self.plot_sample))
        ax.streamplot(
            ds_x,
            ds_y,
            uu,
    ...
    def update(self, observed=None, **kwargs, ellipsoid_matrix):
        ...
        self.particles = self.noise_fn(
            self.dynamics_fn(self.particles, **kwargs), **kwargs, ellipsoid_matrix
        ...
        if observed is not None:
            ...
            if (
                type(observed) == list
                or type(observed) == tuple
                or type(observed) == float
                or type(observed) == int
            ):
        ...
        self.weights_vis = copy.deepcopy(self.weights)
        ...
        if self.n_eff < self.n_eff_threshold:
            ...
            self.weights = np.ones(self.n_particles) / self.n_particles
            self.particle_alive_time = self.particle_alive_time[indices]
            self.particle_alive_time += 1
        ...
        if self.resample_proportion > 0:
            ...
            self.particle_alive_time[random_mask] = 0
